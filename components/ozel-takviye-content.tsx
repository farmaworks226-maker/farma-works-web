...
const hasTableData = (table: TableData | undefined): boolean => {
  ...
  const hasRealData = table.tbody.some((row: TableRow) => row.body.some((cell) => cell.value && cell.value.trim() !== ""))
  return hasRealData
}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#F3EBE2] py-20 text-center border-b border-[#e5d9ca]">
        <div className="container mx-auto px-4">
          ...
          <h1 className="text-4xl font-bold text-[#1E40D8] mb-4">Özel Takviyeler</h1>
          <p className="text-lg text-gray-700 max-w-3xl mx-auto">Sporcular, hamileler ve özel ihtiyaçlar için formüle edilmiş takviye ürünleri.</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-16 max-w-7xl">
        {(!products || products.length === 0) ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 mt-2">Storyblok panelinden &apos;Özel Takviyeler&apos; kategorisine içerik ekleyiniz.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-24">
            {products.map((item) => {
              ...
              return (
                <div key={item.uuid} onClick={() => handleSelectProduct(product)} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col cursor-pointer h-full">
                  <div className="relative h-80 bg-gray-100 overflow-hidden">
                    <span className="absolute top-4 left-4 bg-white/90 text-[#ED6E2D] text-[10px] font-bold px-2 py-1 rounded border border-orange-100 uppercase tracking-wide z-10">{product.category || "Özel Takviye"}</span>
                    <Image src={imageUrl} alt={product.name || "Ürün görseli"} fill className="object-cover transform group-hover:scale-110 transition-transform duration-500" />
                  </div>
      ...
      <div className="bg-[#F3EBE2] py-24">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-[#1E40D8] mb-4">Özel Takviyeler Kimlere Uygundur?</h2>
            <p className="text-lg text-gray-700 max-w-2xl mx-auto">Farklı yaşam dönemleri ve ihtiyaçlar için özel olarak formüle edilmiş takviyeler.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-[#e5d9ca] hover:shadow-md transition duration-300">
              <div className="w-14 h-14 bg-[#F3EBE2] rounded-full flex items-center justify-center mb-6 text-[#ED6E2D]"><Target className="w-8 h-8" /></div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">Sporcular</h3>
              <p className="text-gray-600 leading-relaxed">Performans artırıcı ve kas gelişimini destekleyen özel formüller.</p>
            </div>
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-[#e5d9ca] hover:shadow-md transition duration-300">
              ...
              <p className="text-gray-600 leading-relaxed">Anne ve bebek sağlığı için folik asit ve demir içeren takviyeler.</p>
            </div>
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-[#e5d9ca] hover:shadow-md transition duration-300">
              <div className="w-14 h-14 bg-[#F3EBE2] rounded-full flex items-center justify-center mb-6 text-[#ED6E2D]"><Sparkles className="w-8 h-8" /></div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">Yaşlılar</h3>
              <p className="text-gray-600 leading-relaxed">Kemik ve eklem sağlığını destekleyen özel formülasyonlar.</p>
            </div>
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-[#e5d9ca] hover:shadow-md transition duration-300">
              <div className="w-14 h-14 bg-[#1E40D8]/10 rounded-full flex items-center justify-center mb-6 text-[#1E40D8]"><Leaf className="w-8 h-8" /></div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">Veganlar</h3>
              <p className="text-gray-600 leading-relaxed">Bitkisel kaynaklı B12, demir ve omega-3 takviyeleri.</p>
            </div>
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-[#e5d9ca] hover:shadow-md transition duration-300">
              ...
              <p className="text-gray-600 leading-relaxed">Mevsim geçişlerinde bağışıklık sistemini güçlendiren formüller.</p>
            </div>
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-[#e5d9ca] hover:shadow-md transition duration-300">
              ...
              <p className="text-gray-600 leading-relaxed">Yoğun tempoda çalışanlar için enerji ve konsantrasyon desteği.</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="grid lg:grid-cols-12 gap-10">
                <div className="lg:col-span-5 flex flex-col gap-4">
                  <div className="bg-gray-100 rounded-2xl h-[400px] border border-gray-200 overflow-hidden relative shadow-inner flex items-center justify-center">
                    {activeImage && <Image src={activeImage} alt={selectedProduct.name || "Ürün Detay"} fill className="object-cover" />}
                  </div>
