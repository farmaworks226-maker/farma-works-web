...
const richTextOptions = {
  nodeResolvers: {
    ...
    'td': (children: React.ReactNode, node: { attrs?: { colspan?: number; rowspan?: number } }) => {
      const props: React.TdHTMLAttributes<HTMLTableCellElement> = { className: "px-6 py-3 border-r border-gray-100 last:border-0 text-gray-600" }
      if (node?.attrs?.colspan) props.colSpan = node.attrs.colspan
      if (node?.attrs?.rowspan) props.rowSpan = node.attrs.rowspan
      return <td {...props}>{children}</td>
    },
    'th': (children: React.ReactNode, node: { attrs?: { colspan?: number; rowspan?: number } }) => {
      const props: React.ThHTMLAttributes<HTMLTableCellElement> = { className: "px-6 py-3 font-bold bg-gray-50 border-r border-gray-200 last:border-0 text-gray-800 uppercase text-xs tracking-wider" }
      if (node?.attrs?.colspan) props.colSpan = node.attrs.colspan
      if (node?.attrs?.rowspan) props.rowSpan = node.attrs.rowspan
