...
interface ProductContent {
  ...
  images?: StoryblokImage[]; // Bazı durumlarda gallery yerine images olabilir
...
// --- YARDIMCI FONKSİYONLAR (Optimizasyon için dışarı alındı) ---
...
export function MultivitaminContent({ products }: { products: ProductStory[] }) {
  ...
  // Ürün seçildiğinde yapılacak işlemler
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ÜST BAŞLIK */}
      <div className="bg-lime-50 py-20 text-center border-b border-lime-100">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl font-bold text-lime-900 mb-4">Multivitaminler</h1>
          <p className="text-lg text-lime-800/80 max-w-3xl mx-auto">
            Günlük beslenmenizi destekleyen, enerji ve bağışıklık sisteminizi güçlendiren kapsamlı formüller.
          </p>
        </div>
      </div>
      ...
      <div className="bg-lime-50 py-24">
        <div className="container mx-auto px-4 max-w-6xl">
            <div className="text-center mb-16">
                <h2 className="text-3xl font-bold text-lime-900 mb-4">Multivitaminler Neden Önemlidir?</h2>
                <p className="text-lg text-lime-800/70 max-w-2xl mx-auto">
                    Tek bir tabletle vücudunuzun ihtiyaç duyduğu temel vitamin ve mineralleri karşılayarak genel sağlığınızı destekler.
                </p>
            </div>

            <div className="grid md:grid-cols-3 gap-8">
                <BenefitCard icon={<ShieldCheck className="w-8 h-8" />} color="lime" title="Tam Koruma" desc="Bağışıklık sistemini güçlendiren A, C, D ve E vitaminleri ile çinko gibi mineralleri bir arada sunar." />
                <BenefitCard icon={<Zap className="w-8 h-8" />} color="yellow" title="Maksimum Enerji" desc="B kompleksi vitaminleri, yorgunluk ve bitkinliği azaltarak günlük enerji seviyenizi optimize eder." />
                <BenefitCard icon={<Brain className="w-8 h-8" />} color="indigo" title="Zihinsel Performans" desc="Odaklanma, hafıza ve bilişsel fonksiyonları destekleyen özel mikro besinler içerir." />
                <BenefitCard icon={<Heart className="w-8 h-8" />} color="red" title="Kalp Sağlığı" desc="Kalp fonksiyonlarını destekleyen folik asit, B6 ve B12 vitaminleri ile kalp sağlığınıza katkıda bulunur." />
                <BenefitCard icon={<Activity className="w-8 h-8" />} color="blue" title="Metabolizma Desteği" desc="Besinlerin verimli bir şekilde enerjiye dönüştürülmesi için gerekli olan metabolik süreçleri düzenler." />
                <BenefitCard icon={<Sun className="w-8 h-8" />} color="amber" title="Kemik Gücü" desc="Kalsiyum, magnezyum ve D vitamini kombinasyonu ile kemik yoğunluğunu ve sağlığını korur." />
            </div>
        </div>
      </div>
                   ...
                   <div className="overflow-x-auto my-4 border border-gray-200 rounded-lg shadow-sm">
                      <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                            {selectedProduct.active_ingredients.thead.map((h: { value: string }, k: number) => 
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {selectedProduct.active_ingredients.tbody.map((row: { body: { value: string }[] }, i: number) => (
                            <tr key={i} className="hover:bg-gray-50">
                              {row.body.map((cell: { value: string }, j: number) => (
...
// Yardımcı Bileşen: Fayda Kartı
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function BenefitCard({ icon, color, title, desc }: { icon: any, color: string, title: string, desc: string }) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const colorClasses: any = {
    lime: "bg-lime-100 text-lime-600 border-lime-100",
    yellow: "bg-yellow-100 text-yellow-600 border-lime-100",
    indigo: "bg-indigo-100 text-indigo-600 border-lime-100",
    red: "bg-red-100 text-red-600 border-lime-100",
    blue: "bg-blue-100 text-blue-600 border-lime-100",
    amber: "bg-amber-100 text-amber-600 border-lime-100",
  }
  
  const classes = colorClasses[color] || colorClasses.lime;
  
  return (
    <div className={`bg-white p-8 rounded-2xl shadow-sm border hover:shadow-md transition duration-300 ${classes.split(" ")[2]}`}>
        <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-6 ${classes.split(" ").slice(0, 2).join(" ")}`}>
            {icon}
        </div>
