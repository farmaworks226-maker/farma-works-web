...
export function BalikYaglariContent({ products }: { products: StoryblokStory<Product>[] }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-br from-blue-50 via-cyan-50 to-sky-50 py-20 text-center border-b border-blue-100">
        <div className="container mx-auto px-4">
          <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-700 px-4 py-2 rounded-full text-sm font-medium mb-6">
            <Droplets className="w-4 h-4" />
            Omega-3 Zengin
          </div>
          <h1 className="text-4xl font-bold text-blue-900 mb-4">Balık Yağları</h1>
          <p className="text-lg text-blue-800/80 max-w-3xl mx-auto">Kalp, beyin ve eklem sağlığınız için yüksek saflıkta EPA ve DHA içeren premium balık yağı takviyeleri.</p>
        </div>
      </div>
              ...
              return (
                <div key={item.uuid} onClick={() => setSelectedProduct(product)} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col cursor-pointer h-full">
                  <div className="relative h-80 bg-gradient-to-br from-blue-50 to-cyan-50 overflow-hidden">
                    <span className="absolute top-4 left-4 bg-white/90 text-blue-600 text-[10px] font-bold px-2 py-1 rounded border border-blue-100 uppercase tracking-wide z-10">{product.category || "Balık Yağları"}</span>
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto pt-4 border-t border-gray-50">
                      <span className="text-blue-600 text-sm font-bold flex items-center group-hover:translate-x-1 transition-transform">İncele <ArrowRight className="w-4 h-4 ml-1" /></span>
                    </div>
                  </div>
                </div>
              )
      ...
      {/* BALIK YAĞI FAYDALARI */}
      <div className="bg-gradient-to-br from-blue-50 via-cyan-50 to-sky-50 py-24">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-blue-900 mb-4">Omega-3 Balık Yağının Faydaları</h2>
            <p className="text-lg text-blue-800/70 max-w-2xl mx-auto">EPA ve DHA açısından zengin balık yağları, vücudunuzun birçok sistemini destekleyen güçlü besinlerdir.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-blue-100 hover:shadow-md transition duration-300">
              <div className="w-14 h-14 bg-red-100 rounded-full flex items-center justify-center mb-6 text-red-500"><Heart className="w-8 h-8" /></div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-blue-100 hover:shadow-md transition duration-300">
              <div className="w-14 h-14 bg-purple-100 rounded-full flex items-center justify-center mb-6 text-purple-600"><Brain className="w-8 h-8" /></div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-blue-100 hover:shadow-md transition duration-300">
              <div className="w-14 h-14 bg-cyan-100 rounded-full flex items-center justify-center mb-6 text-cyan-600"><Eye className="w-8 h-8" /></div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-blue-100 hover:shadow-md transition duration-300">
              <div className="w-14 h-14 bg-orange-100 rounded-full flex items-center justify-center mb-6 text-orange-500"><Bone className="w-8 h-8" /></div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-blue-100 hover:shadow-md transition duration-300">
              <div className="w-14 h-14 bg-teal-100 rounded-full flex items-center justify-center mb-6 text-teal-600"><Droplets className="w-8 h-8" /></div>
            ...
            <div className="p-6 md:p-10">
              ...
              <div className="grid lg:grid-cols-12 gap-10">
                <div className="lg:col-span-5 flex flex-col gap-4">
                  <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl h-[400px] border border-blue-200 overflow-hidden relative shadow-inner flex items-center justify-center">
                  ...
                  {galleryList.length > 1 && (
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                      {galleryList.map((imgUrl, i) => (
                        <button key={i} onClick={() => setManualActiveImage(imgUrl)} className={`relative w-20 h-20 rounded-lg overflow-hidden border-2 flex-shrink-0 bg-white transition-all ${activeImage === imgUrl ? "border-blue-600 ring-2 ring-blue-600/20" : "border-gray-100 hover:border-gray-300"}`}>
                ...
                <div className="lg:col-span-7 space-y-8">
                  ...
                  {hasData(selectedProduct.net_quantity) && (
                    <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-600 px-5 py-2.5 rounded-lg font-bold border border-blue-100 shadow-sm"><Tag className="w-5 h-5" /> Net Miktar: {selectedProduct.net_quantity}</div>
                  )}
                </div>
              </div>
              <div className="mt-12 space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  {hasData(selectedProduct.ingredients) && (
                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
                      <div className="flex items-center gap-2 font-bold text-gray-900 mb-3 text-lg"><Info className="w-6 h-6 text-blue-600" /> İçerik</div>
                  ...
                  {hasData(selectedProduct.usage) && (
                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
                      <div className="flex items-center gap-2 font-bold text-gray-900 mb-3 text-lg"><AlertCircle className="w-6 h-6 text-blue-600" /> Kullanım Şekli</div>
                  ...
                  <div className="overflow-x-auto my-4 border border-gray-200 rounded-lg shadow-sm">
                    <table className="w-full text-sm text-left">
                      <thead className="bg-blue-50 border-b border-blue-200">
                        <tr>{(selectedProduct.active_ingredients as TableData).thead?.map((h, k) => <th key={k} className="px-6 py-3 font-bold text-blue-900">{h.value}</th>)}</tr>
                      </thead>
                ...
                {hasData(selectedProduct.price) && (
                  <div className="bg-blue-50 border border-blue-200 p-6 rounded-xl flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-4"><div className="bg-white p-3 rounded-full shadow-md text-blue-600"><Tag className="w-6 h-6" /></div><div className="font-bold text-blue-900 text-lg">Satış Fiyatı:</div></div>
                    <div className="text-2xl font-extrabold text-blue-600">{selectedProduct.price}</div>
                  </div>
                )}
              </div>
            </div>
